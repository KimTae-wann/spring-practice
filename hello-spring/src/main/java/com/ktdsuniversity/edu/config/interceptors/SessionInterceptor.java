 ...
 * 9. 회원 탈퇴 처리 URL
 * 10. 회원 로그아웃 URL
 * ==> 위 URL 10개에 세션없이 접근할 경우 로그인 페이지로 이동시키기. (컨트롤러가 실행되기 전)
 * 
 ...
 * 8. 이메일 중복 검사 URL
 * 9. 첨부파일 다운로드 URL
 * ==> 위 URL 9개중 1 ~ 4 까지는 세션이 있는 상태에서 접근을 못해야 한다.
 */
...
public class SessionInterceptor implements HandlerInterceptor{
	 ...
	 * @return 컨트롤러의 실행 여부를 반환 (true: 컨트롤러를 실행, false: 컨트롤러를 실행하지 않는다!)
	 */
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (session.getAttribute("__LOGIN_DATA__") == null) {
			...
			String pathname = request.getRequestURI(); // location에 있는 pathname과 동일하다 /api/... 
			...
			// redirect:/login 은 스프링 컨트롤러의 리턴 값에서 사용하는 문법이라 사용 불가능하다.
			// HttpServletResponse 객체에서는 아래와 같이 사용해야 한다.
	...
	/**
	 * 컨트롤러가 실행된 이후에 수행할 공통 코드를 작성하는 영역 (View가 렌더링 되기 전)
	...
	/**
	 * JSP를 HTML로 변환시켜 브라우저(클라이언트)로 반환하기 직전에 수행되는 영역 (모든 작업이 끝난 후)
